<template>
  <div class="flex flex-col h-screen overflow-hidden relative">
    ...
    <header class="px-6 pt-8 pb-6 flex flex-col gap-4 sticky top-0 z-30 bg-background-dark/80 glass border-b border-white/5">
      <div class="flex justify-between items-center">
        <h1 class="text-3xl font-bold tracking-tight">Bar Sets</h1>
      </div>
      <div class="relative group">
        ...
        <input 
          v-model="searchQuery"
          class="w-full bg-slate-900/60 border border-slate-800/50 rounded-xl py-2.5 pl-10 pr-4 text-sm focus:ring-2 focus:ring-primary focus:bg-slate-900 transition-all placeholder:text-slate-500" 
    ...
    <main class="flex-1 overflow-y-auto hide-scrollbar">
      <div class="px-6 pt-8 pb-32">
        ...
        <section class="mb-8">
          <h2 class="text-xs font-bold uppercase tracking-widest text-slate-500 mb-4 px-1">Create New Bar Set</h2>
          <Create @create="create" />
        </section>

        <section>
          <h2 class="text-xs font-bold uppercase tracking-widest text-slate-500 mb-4 px-1">Saved Bar Sets</h2>
    ...
    <div class="fixed bottom-0 left-0 right-0 z-40 bg-slate-900/60 glass border-t border-slate-800/50 shadow-[0_-10px_40px_rgba(0,0,0,0.6)]">
      <div class="pb-4 pt-3 px-6">
        <div class="flex flex-col items-center">
          <div class="w-10 h-1 bg-slate-700/60 rounded-full mb-4"></div>
          <div class="w-full flex items-center justify-between gap-4">
            <div class="flex items-center gap-3">
              <div class="relative flex items-center justify-center w-8 h-8 rounded-lg bg-primary/10">
                <span class="material-symbols-outlined text-primary text-xl">cloud_sync</span>
                <div v-if="driveConnected" class="absolute top-1.5 right-1.5 w-2 h-2 rounded-full bg-emerald-500 border-2 border-slate-900"></div>
              </div>
              <div class="flex flex-col">
                <span class="text-[10px] uppercase tracking-wider font-bold text-slate-500">Backup Status</span>
                <span class="text-xs font-medium text-slate-300">{{ backupStatusText }}</span>
              </div>
            </div>
            <button 
              @click="showBackupPanel = !showBackupPanel"
              class="text-xs font-bold text-primary hover:text-white transition-all flex items-center gap-2 px-4 py-2 rounded-xl bg-primary/10 border border-primary/20 hover:bg-primary active:scale-95"
            >
